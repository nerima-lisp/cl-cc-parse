  ...
  (flet ((lower-one (sexp)
    ...
    (let* ((desc          (assoc op *arith-op-descriptors*))
      ...
      (cond
        ((null operands)
         identity-node)
        ((and (null (cdr operands)) unary-prefix-p)
         (binop-ast identity-node (lower-one (car operands))))
        ((null (cdr operands))
         (lower-one (car operands)))
        (t
...
;;; Setf compound place lowering is in lower-setf.lisp (loads after).
